...
> MEMO：：**nグラム**の２文字分割が最強説。どんな文字にでもマッチする確率が高い。表記ゆれ、未知の単語にもマッチする。
> **nグラム　OR　形態素解析　調べる**
>
**TODO::このPOCの目的「何を検証するのか」、「確認項目」を明確にする。検証項目を列挙すること。**
同じ処理が複数個所に記載されているときに拾えるのか。
意味的に似ているものを一括りにできるか、ベクトルＤＢで実現できるのか。プログラムでもあるのか
チャンクが重要。関数ごとならいけるかも？
...
TODO::検証項目：プロンプト最適化
TODO（今後の拡張）::会話の履歴をプロンプトに設定する。要約か、全文か。精度検証する。会話の保存領域を作る。
前処理で呼び出しと定義も一緒に
...
【やってほしいこと】TODO::追記する
ユーザの質問に対してプロンプトを渡すので

【厳守事項】
- 検索結果に記載された情報のみを使用する
- ~~推測・補完・一般知識の使用は禁止~~
- 推測の場合は推測であるということが明確にわかる文章で返す
...
- Top-K動的調整（大規模言語モデル（LLM）が次のトークン（単語や文字）を生成する際に、候補となる単語の個数（K）を固定せず、文脈や確率分布の形状に応じて動的に変更する手法）
...
====================================
①1回で返す
②会話の要約も一緒に（ユーザの指示で関連ファイルをとる）★調整ポイント
③関連ソースも一緒に
ベクトルDBで有効なのか検証する


AIの進化のスピードについていくため
AIを使って業務を効率化するシステム作り＝システム屋の最後の仕事